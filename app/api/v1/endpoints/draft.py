...
async def get_latest_drafts_endpoint(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user_data),
    lang: str = Query("en", description="Language of the response, either 'en' for English or 'km' for Khmer")
):
    try:
        drafts = await get_latest_drafts_per_assessment_type(db, current_user, lang)

        return BaseResponse(
            status=200,
            message="Latest drafts retrieved successfully." if lang == "en" else "ការទាញយកការតេស្តចុងក្រោយបានជោគជ័យ។",
            date=datetime.utcnow().strftime("%d-%B-%Y %H:%M:%S"),
            payload=drafts
        )
    except HTTPException as e:
        ...
        raise format_http_exception(
            status_code=400,
            message="Failed to retrieve the latest drafts." if lang == "en" else "បរាជ័យក្នុងការទាញយកតេស្តចុងក្រោយ។",
            details=str(e),
        )
