...
async def get_jobs(token: str = Depends(oauth2_scheme)):
...
async def get_job_details(uuid: str, token: str = Depends(oauth2_scheme)):
    headers = {"Authorization": f"Bearer {token}"}
    async with httpx.AsyncClient() as client:
        try:
...
async def delete_job(uuid: str, token: str = Depends(oauth2_scheme)):
    headers = {"Authorization": f"Bearer {token}"}
    async with httpx.AsyncClient() as client:
        try:
